...
| P10.23 | **`campaigns: quantify serial evidence`** | **CURRENT PR.** Add bounded occurrence predicates for text, regex, and JSON-lines evidence, with strict validation and identical evaluation in properties, operation guards, checkpoint pruning, replay/minimization/reporting, and the self-contained Compose tutorial. | Rich event schemas, arbitrary query languages, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
