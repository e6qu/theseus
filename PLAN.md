...
| P11.10 | **`release: cache native runtime builds`** | **CURRENT PR.** Reuse isolated GitHub Actions BuildKit caches for each native runtime architecture, export full dependency layers after successful builds, and tolerate cache-service outages without compromising publication. | Reproducible binary timestamps. |
