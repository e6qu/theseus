...
| P10.9 | **`campaigns: model operation state rules`** | **CURRENT PR.** Let operations exclude named prior operations and cap their own use count. Validate declarative state rules during Compose normalization and generate only histories that satisfy requirements, exclusions, and bounds while retaining deterministic coverage, faults, replay, minimization, checkpoints, reports, and the self-contained tutorial. | Generalized operation grammar, marker/data-dependent guards, guest-PC coverage, probabilistic/RL guidance, or UFFD-backed CoW memory. |
