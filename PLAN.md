...
| P10.26 | **`campaigns: correlate ordered JSON events`** | **CURRENT PR.** Let JSON-lines sequence items capture a JSON Pointer value and require a later event pointer to equal that capture. Preserve strict ordering and capture validation, property/guard evaluation, replay/minimization/reporting, and a self-contained Compose tutorial proving a transaction correlation. | Arbitrary query languages, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
