...
| P12.19 | **`topology: retain scheduler budget evidence`** | **CURRENT PR.** Record the final scheduler round and shared round budget in topology results, so a locked run proves whether it stopped naturally or at its deterministic execution limit. | Hardware instruction tracing, RL guidance, or UFFD-backed CoW memory. |
