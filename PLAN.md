...
| P10.45 | **`campaigns: render checkpoint-captured inputs`** | **CURRENT PR.** Let an operation render a UART input template from scalar JSON values in the latest matching restored serial event. Validate capture sources and placeholders; preserve concrete per-run replay bytes, checkpoint reuse, reports, minimization, and the self-contained Compose tutorial. | Multi-event input composition, arbitrary query languages, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
