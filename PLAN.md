...
| P10.30 | **`campaigns: join structured events on composite keys`** | **CURRENT PR.** Let JSON correlation and join endpoints require a shared tuple of JSON Pointer values, not just one value. Preserve backward-compatible replay reads, strict pointer-set validation, property/operation join evaluation, reporting, and a self-contained Compose tutorial joining request ID plus attempt. | Arbitrary query languages, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
