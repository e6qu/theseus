...
| P11.3 | **`release: make archive checksums portable`** | **CURRENT PR.** Generate Linux and macOS `SHA256SUMS` from inside each bundle so paths are valid after extraction, and keep the release's archive verification as a consumer would run it. | Post-publish external consumer smoke test. |
