...
| P10.17 | **`campaigns: match structured serial events`** | **CURRENT PR.** Add JSON-lines predicate leaves that require exact JSON-Pointer values on one complete serial event. Normalize and validate pointers, evaluate them in campaigns, reports, minimization, and replay checks, and demonstrate them in the self-contained Compose tutorial. | Rich event schemas, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
