...
| P12.10 | **`campaigns: complete browser boundary evidence`** | **CURRENT PR.** Render the scheduler round, changed storage, virtual-time delta, and checkpoint state hash in the offline browser report, matching the complete Markdown and machine-JSON timeline while retaining safe defaults for older result files. | Hardware instruction tracing, RL guidance, or UFFD-backed CoW memory. |
