...
| P10.27 | **`campaigns: correlate structured events across services`** | **CURRENT PR.** Let campaign properties require a JSON Pointer value captured from one service event to equal a pointer in another service event. Preserve strict endpoint/service/pointer validation, property evaluation, replay/minimization/reporting, and a self-contained Compose tutorial proving an API-to-auditor transaction join. | Arbitrary query languages, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
