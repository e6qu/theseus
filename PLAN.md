...
| P11.15 | **`release: attest independent rebuild witnesses`** | **CURRENT PR.** Let an external GitHub repository verify the official signed input record, rebuild each native runtime twice without cache, compare it to the published digest, and sign a retained witness artifact. | Broader source-dependency pinning. |
