...
| P8.11 | **`network: add deterministic frame jitter`** | Add a seeded, per-frame scheduler-round delay that can reorder simulated-network delivery without using host time. | Wall-clock timers, bandwidth limits, or random host scheduling. |

**Current PR: P8.11 only.** The manifest is an execution contract, not a second
