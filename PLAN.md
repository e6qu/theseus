...
| P12.24 | **`topology: schedule restart event barriers by rounds`** | **CURRENT PR.** Re-deliver restart events through causal, post-input serial barriers driven by deterministic service and network rounds. Remove the last host-time serial polling path from normal topology execution. | Hardware instruction tracing, RL guidance, or UFFD-backed CoW memory. |
