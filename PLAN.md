...
| P10.28 | **`campaigns: join JSON events across service sets`** | **CURRENT PR.** Let a property require one JSON Pointer value to occur in every endpoint of an arbitrary service set. Preserve strict endpoint-count/service/pointer validation, common-value semantics, replay/minimization/reporting, and a self-contained Compose tutorial joining API, replica, and auditor transaction events. | Arbitrary query languages, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
