...
| P10.33 | **`campaigns: reuse named serial evidence`** | **CURRENT PR.** Define and compose named recursive serial-evidence trees once per campaign, then expand `use: name` references from properties, operation checkpoint guards, and nested definitions. Reject unknown and cyclic definitions while locking expanded self-contained replay plans. | Arbitrary query languages, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
