...
| P10.32 | **`campaigns: relate structured event values`** | **CURRENT PR.** Add JSON endpoint relations to serial-evidence trees: equality/inequality for scalar or composite keys and numeric ordering for scalar values. Apply the same existential relation semantics to property evaluation and operation checkpoint pruning, with strict arity validation and a self-contained Compose tutorial. | Arbitrary query languages, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
