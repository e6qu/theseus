...
| P10.31 | **`campaigns: compose cross-service serial evidence`** | **CURRENT PR.** Add recursive `all`/`any`/`none` expressions over service-scoped serial guards, JSON correlations, and composite JSON joins. Apply required and excluded expressions consistently to properties and operation checkpoint pruning; preserve replay/minimization/reporting and demonstrate the complete self-contained Compose flow. | Arbitrary query languages, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
