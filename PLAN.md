...
| P10.22 | **`campaigns: require ordered serial-event sequences`** | **CURRENT PR.** Add a direct `sequence` predicate for ordered serial text, regex, and JSON-lines events, and carry it through campaign properties and operation guards, prefix pruning, replay/minimization/reporting, and the self-contained Compose tutorial. | Rich event schema/quantifiers, arbitrary query languages, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
