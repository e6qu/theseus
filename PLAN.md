...
| P10.40 | **`campaigns: explore named operation input matrices`** | **CURRENT PR.** Let one logical operation declare named UART input cases. Explore each case with stable breadth-first scheduling while keeping requirements, stages, use limits, fault barriers, checkpoint reuse, reports, minimization, and locked replay attached to the logical operation. | Generalized operation grammars, arbitrary query languages, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
