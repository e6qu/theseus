...
| P9.3 | **`product: bounded campaign fault sequences`** | **CURRENT PR.** Campaigns combine compatible candidates in stable declaration order, up to `max_faults_per_run` (default 2; cap 4) and then apply `max_runs` to the complete corpus. Campaign results, reports, minimization, and legacy single-fault bundles all preserve or read the full sequence. | Copy-on-write whole-topology VM snapshots, unconstrained/coverage-guided sequence search, packet-match rules, storage recovery actions, instruction-exact time, or large-scale RL guidance. |
