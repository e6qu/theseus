...
| P10.15 | **`campaigns: compose nested serial predicates`** | **CURRENT PR.** Add recursive `all`, `any`, and `none` serial-predicate groups. Normalize and validate the expression tree, evaluate it per service transcript, lock it into minimization and replay checks, and use it in the self-contained Compose tutorial. | Regex or structured-event predicates, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
