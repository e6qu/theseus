...
| P10.10 | **`campaigns: guard operations on observed markers`** | **CURRENT PR.** Let an operation require or exclude markers actually emitted before its parent checkpoint. Validate the guard declarations, restore and inspect the exact prefix state before extending it, skip impossible leaves without spending campaign-run budget, lock selected histories for replay, and render the dynamic guard model in reports and the self-contained tutorial. | Generalized operation grammar, guest-PC coverage, probabilistic/RL guidance, or UFFD-backed CoW memory. |
