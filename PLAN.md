...
| P10.51 | **`campaigns: capture ordered JSON transactions`** | **CURRENT PR.** Let a captured input select its value from the final JSON event of a correlated ordered sequence. Preserve source validation, checkpoint rendering, reports, replay, minimization, and the self-contained Compose tutorial. | Arbitrary query languages, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
