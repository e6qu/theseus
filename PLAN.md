...
| P12.18 | **`topology: bound execution by scheduler rounds`** | **CURRENT PR.** Let each service declare a bounded `max_rounds` execution budget and stop topology runs at the shared deterministic round limit rather than host elapsed time. | Hardware instruction tracing, RL guidance, or UFFD-backed CoW memory. |
