...
| P12.30 | **`topology: replay-check lifecycle rounds`** | **CURRENT PR.** Verify each replay's lifecycle barrier round total against the recorded topology result, so the global scheduling budget is durable replay evidence. | Hardware instruction tracing, RL guidance, or UFFD-backed CoW memory. |
