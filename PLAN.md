...
| P12.5 | **`campaigns: show serial evidence at operation boundaries`** | **CURRENT PR.** Record a deterministic per-service serial-byte delta at every operation boundary, with an escaped bounded excerpt, byte count, and full-delta hash. Replay-check it and render it beside the existing checkpoint evidence, so people can see what changed without opening raw logs while every byte remains auditable in the locked bundle. | Hardware instruction tracing, RL guidance, or UFFD-backed CoW memory. |
