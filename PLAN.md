...
| P10.49 | **`campaigns: select captured input occurrences`** | **CURRENT PR.** Let each captured input explicitly select the first or latest matching restored event; keep latest as the compatible default. Preserve validation, rendered replay bytes, reports, minimization, and the self-contained Compose tutorial. | Arbitrary query languages, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
