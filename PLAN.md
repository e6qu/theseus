...
| P12.14 | **`campaigns: preserve delivered input evidence`** | **CURRENT PR.** Record a bounded, escaped excerpt, byte count, and full hash of each operation's exact UART input beside its replay-checked target boundary. Render it in portable reports, retain older-bundle compatibility, and explain the direct delivery record in the self-contained Compose tutorial. | Hardware instruction tracing, RL guidance, or UFFD-backed CoW memory. |
