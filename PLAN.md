...
| P11.11 | **`release: normalize reproducible artifact timestamps`** | **CURRENT PR.** Derive a release-wide source date from the signed commit; apply it to Rust, kernel, module, gzip, tar, and SDK packaging; and use one portable archive writer so Linux and macOS archives have ordered paths and normalized ownership. | Pin the kernel upstream revision and prove whole-image bit reproducibility. |
