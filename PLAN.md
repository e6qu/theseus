...
| P10.43 | **`campaigns: model protocol state machines`** | **CURRENT PR.** Let a campaign declare finite initial state; operations and input cases can require values and set new values. Validate every state key, preserve deterministic scheduling, case specialization, fault barriers, checkpoints, reports, minimization, replay, and the self-contained Compose tutorial. | Generalized operation grammars, arbitrary query languages, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
