...
| P10.46 | **`campaigns: combine captured and grammar inputs`** | **CURRENT PR.** Let finite input grammars combine stable named choices with checkpoint-captured JSON values. Preserve bounded case expansion, source validation, exact rendered replay bytes, state transitions, fault barriers, reports, minimization, and the self-contained Compose tutorial. | Typed input encodings, arbitrary query languages, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
