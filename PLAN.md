...
| P9.1 | **`product: barrier-triggered topology faults`** | **CURRENT PR.** Campaign candidates can now `partition` or `heal` every simulated NIC on one named Compose network, or apply deterministic error/latency/torn-write/read-corruption settings to one simulated drive, immediately after a named UART operation checkpoint. Action history is retained in campaign results, static replay plans, minimization, and replay comparison; tutorial 10 uses all three forms. | Copy-on-write whole-topology VM snapshots, link-direction/packet-specific rules, storage recovery actions, instruction-exact time, or large-scale RL guidance. |
