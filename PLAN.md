...
| P10.25 | **`campaigns: query structured event arrays`** | **CURRENT PR.** Let JSON-lines serial predicates require that any, all, or none of the records at a JSON Pointer array match a nested JSON predicate. Preserve strict recursive validation, property/guard evaluation, replay/minimization/reporting, and a self-contained Compose tutorial. | Arbitrary query languages, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
