...
| P12.8 | **`campaigns: show deterministic scheduler progress`** | **CURRENT PR.** Record the scheduler round for every operation boundary and include it in the checkpoint identity and portable report, so timing progress remains explicit without relying on host wall time. | Hardware instruction tracing, RL guidance, or UFFD-backed CoW memory. |
