...
| P10.47 | **`campaigns: encode captured input values`** | **CURRENT PR.** Let captured scalar values render as text, JSON literals, or hexadecimal text. Validate and lock the selected representation through checkpoint rendering, reports, replay, minimization, and the self-contained Compose tutorial. | Arbitrary query languages, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
