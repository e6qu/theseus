...
| P12.6 | **`campaigns: show topology effects at operation boundaries`** | **CURRENT PR.** Record per-service simulated-network counter deltas at every operation boundary. Replay-check and render TX, RX, drop, duplicate, and corruption effects beside serial and paused-PC evidence, so an operation's external topology effect is visible without reconstructing it from raw logs. | Hardware instruction tracing, RL guidance, or UFFD-backed CoW memory. |
