...
| P10.53 | **`campaigns: query JSON events with JSONPath`** | **CURRENT PR.** Let every structured JSON event predicate require an RFC 9535 JSONPath query to select a node from the same event. Preserve strict source validation, properties, operation guards, captured inputs, reports, replay, minimization, and the self-contained Compose tutorial. | Instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
