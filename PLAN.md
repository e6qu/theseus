...
| P10.39 | **`campaigns: map workflow keys across schemas`** | **CURRENT PR.** Let every workflow stage override the JSON pointers used for its shared key while requiring equal tuple width. Preserve local path ordering, cardinality, properties, operation checkpoint pruning, reports, replay plans, and the self-contained Compose tutorial. | Arbitrary query languages, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
