...
| P10.16 | **`campaigns: match serial predicates with regex`** | **CURRENT PR.** Add validated Rust-regex leaves to nested serial predicates. Evaluate the same byte-level expression in campaigns, reports, minimization, and replay checks, and demonstrate it in the self-contained Compose tutorial. | Structured-event predicates, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
