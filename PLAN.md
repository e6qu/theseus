...
| P10.54 | **`campaigns: guide with live guest instruction locations`** | **CURRENT PR.** Use deterministic paused-vCPU program-counter samples as per-service instruction-location coverage when selecting campaign extensions. Preserve selection reasons, replay verification, reports, and the self-contained Compose tutorial without claiming a full instruction trace. | Probabilistic/RL guidance or UFFD-backed CoW memory. |
