...
| P10.55 | **`campaigns: adapt to observed operation yield`** | **CURRENT PR.** Add an opt-in deterministic adaptive policy that combines prefix coverage with each operation case's observed marker, instruction-location, topology-state, and failure yield, plus a declining exploration bonus. Lock policy and exact reasons into reports and replay; demonstrate it in the self-contained Compose tutorial. | Full instruction traces, probabilistic/RL guidance, or UFFD-backed CoW memory. |
