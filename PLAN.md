...
| P9.8 | **`product: IPv4 transport campaign packet loss`** | **CURRENT PR.** Packet campaigns can match IPv4 protocol and TCP/UDP source or destination ports, with narrower selectors taking precedence over broad EtherType rules. | IP options beyond header length, IPv6, payload filters, copy-on-write whole-topology VM snapshots, or coverage-guided sequence search. |
