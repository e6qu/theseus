...
| P10.14 | **`campaigns: admit alternative serial property evidence`** | **CURRENT PR.** Let a campaign property require at least one of several serial markers alongside its required and forbidden evidence. Validate declarations, preserve the predicate in campaign reports, minimization, and replay checks, and demonstrate it in the self-contained Compose tutorial. | Nested Boolean expressions, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
