...
| P10.41 | **`campaigns: constrain input-case transitions`** | **CURRENT PR.** Let named input cases require or exclude a logical operation or one exact earlier case, and cap their own repetitions. Validate references and unreachable case graphs; preserve stable scheduling, fault barriers, checkpoints, reports, minimization, locked replay, and the self-contained Compose tutorial. | Generalized operation grammars, arbitrary query languages, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
