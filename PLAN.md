...
| P9.6 | **`product: EtherType-matched campaign packet loss`** | **CURRENT PR.** Campaign candidates can apply deterministic packet loss only to Ethernet frames matching one declared EtherType, then remove that rule with `packet_recover` without disturbing ordinary packet conditions, partitions, links, traffic evidence, queues, or seeded state. Actions stay in reports, minimization, and replay verification. | IP/TCP/payload filters, copy-on-write whole-topology VM snapshots, unconstrained/coverage-guided sequence search, instruction-exact time, or large-scale RL guidance. |
