...
| P10.34 | **`campaigns: require every structured event join`** | **CURRENT PR.** Add `quantifier: every` to JSON joins so every key selected from the first endpoint must occur at every peer endpoint, while preserving `any` as the backward-compatible existential default. Apply it to properties, operation checkpoint pruning, reports, replay, and the self-contained Compose tutorial. | Arbitrary query languages, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
