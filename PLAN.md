...
| P10.24 | **`campaigns: join multi-service property evidence`** | **CURRENT PR.** Let campaign properties require all or any, and exclude any, service-scoped serial guards. Validate every service, evaluate the same evidence during campaign results, minimization, and replay, render it in property detail, and demonstrate it in the self-contained Compose tutorial. | Arbitrary query languages, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
