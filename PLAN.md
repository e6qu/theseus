...
| P10.13 | **`campaigns: compose serial property predicates`** | **CURRENT PR.** Extend campaign properties with required and forbidden companion serial evidence, validate predicate declarations, evaluate them within one service transcript for every selected timeline, and retain that predicate through minimization. | General predicate expressions, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
