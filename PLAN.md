...
| P10.52 | **`campaigns: capture cross-service workflows`** | **CURRENT PR.** Let a captured input select its terminal value only when a keyed transaction completes ordered stages across named services. Preserve source validation, checkpoint rendering, reports, replay, minimization, and the self-contained Compose tutorial. | Arbitrary query languages, instruction coverage for live Compose guests, probabilistic/RL guidance, or UFFD-backed CoW memory. |
