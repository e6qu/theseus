...
| P11.7 | **`release: authenticate OCI provenance consumers`** | **CURRENT PR.** Give every runtime consumer the workflow token before it verifies the bare multi-architecture image's provenance, rather than only before later GitHub Release downloads. | SBOMs and release retention policy. |
