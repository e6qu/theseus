...
| P12.27 | **`topology: share the restart barrier budget`** | **CURRENT PR.** Spend each replacement service's deterministic round budget across readiness and every replayed UART barrier, rather than granting each barrier a fresh full allowance. | Hardware instruction tracing, RL guidance, or UFFD-backed CoW memory. |
