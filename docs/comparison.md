...
| Replay | Instruction-level deterministic reproduction | Locked-input replay through recorded guest exit. Fixed runs enforce the complete KVM/device stream; checkpoint-backed campaigns enforce host inputs and interrupt deliveries while retaining intervening exits as evidence. Instruction execution, interrupt servicing, and in-kernel timers remain uncontrolled; host-timed cutoffs are not replayable exits. |
