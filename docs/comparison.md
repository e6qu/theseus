...
| Replay | Instruction-level deterministic reproduction | Locked-input replay with an actively enforced machine stream for KVM exits, device effects, UART/control input, clock jumps, and supported userspace device interrupt injection; instruction execution, interrupt servicing, and in-kernel timers remain uncontrolled |
