...
| Replay | Instruction-level deterministic reproduction | Locked-input replay with an actively enforced machine stream for KVM exits, device effects, UART/control input, clock jumps, and UART interrupt injection; instruction execution, interrupt servicing, timers, and non-UART interrupts remain uncontrolled |
