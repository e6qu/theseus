...
| Replay | Instruction-level deterministic reproduction | Locked-input replay with an actively enforced machine stream for KVM exits, device effects, UART/control input, and clock jumps; execution and interrupt delivery between exits remain uncontrolled |
