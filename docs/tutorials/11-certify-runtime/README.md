...
The first event waits until the complete `finished` line reaches the host. Only
then does Theseus send `shutdown`, allowing the guest to reset. This explicit
handshake keeps the terminal boundary independent of host thread timing.
