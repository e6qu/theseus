...
guests, connects them to the `backplane` network, and checks that `api` can
ping `worker`.
...
its own Theseus manifest. `theseus compose test` locks the artifacts, runs the
two guests in one deterministic topology, and leaves their serial logs and
results in `theseus-compose-replay/services/`.
