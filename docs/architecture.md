...
| Execution stream | `firecracker/vmm` + `topology-runner` + `cli` | Per-vCPU rolling identities plus one VM-wide total order for handled KVM exits, emulated device effects, UART/control input, and virtual-clock jumps, inherited across branches and actively enforced during replay. |
