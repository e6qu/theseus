...
| Workload packaging | Docker Compose and Kubernetes (single-node K3s, `kapp` readiness, air-gapped) | Compose subset over images and manifests; documented Kubernetes subset (single-container Pods/Deployments, ClusterIP Services) translating into the same locked plan | Kubernetes controllers beyond Deployments, ConfigMaps/Secrets as files, wider Compose breadth |
