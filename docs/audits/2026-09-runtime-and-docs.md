...
| Ordered execution evidence | Implemented, awaiting published runtime evidence | Each vCPU retains a rolling digest and readable tail of handled KVM exits. Checkpoint children inherit the ledger; replay, comparison, reports, and runtime certification reject divergence. Tutorial 41 is the public ordinary-container path. The ledger detects exit-boundary drift but does not yet control instruction, thread, or interrupt order. |
