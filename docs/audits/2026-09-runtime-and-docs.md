...
| Test-command lifecycle | Implemented, awaiting published runtime evidence | Compose accepts explicit lifecycle operations or discovers one selected Antithesis-compatible template across locked service images. Generated histories enforce setup and exclusivity, choose bounded parallel slots, join final timelines, kill live commands for eventual checks, and recover active campaign faults for quiet terminal checks. Paths and decisions remain in plans, traces, replay, minimization, and reports. Tutorial 40 is the public runtime path. Multiple-template selection and production-scale validation remain absent. |
