...
| Ordered execution replay | Implemented, awaiting published runtime evidence | Each vCPU retains a local rolling ledger and every VM retains one bounded exact machine-wide stream. Checkpoint children inherit both forms. Replay gates each emulated device-effect boundary and explicit UART, control-channel, or virtual-clock input on the recorded actor and payload; comparison, reports, and runtime certification retain the evidence. Tutorial 41 is the public ordinary-container path. Instruction, Linux thread/process, timer, interrupt delivery, and guest-side input consumption between exits remain uncontrolled. |
