...
| Branch snapshots | Implemented | Capture copies all guest RAM into a memfd. Children restore with private copy-on-write mappings. The full path is not zero-copy. Compose's explicit `ready_checkpoint` contract can retain a durable whole-topology root with locked state/RAM and NIC/switch/scheduler/execution context. Tutorial 11's v5 certificate proves only the resumed suffix; boot is ancestry. First startup divergence is retained separately. Published portfolio evidence remains required; a later restart can introduce an uncontrolled boot. |
| Ready-checkpoint replay | Implemented, awaiting published runtime evidence | Version-3 single-service bundles retain full state/RAM, the inherited trace, device/control context, and pending userspace interrupts. The baseline and replays restore the same checkpoint and gate the resumed suffix. Tutorial 42 reads a UART and both standard random devices directly from retained Linux state. Boot/prelude output is separate; boot is inherited, not replayed. The workflow excludes live I/O devices and UART/RNG rate limiting, requires compatible native Linux/KVM, and does not add instruction or kernel-timer control. |
...
| Test-command lifecycle | Implemented, awaiting published runtime evidence | Compose accepts explicit lifecycle operations or discovers all or a named subset of Antithesis-compatible templates across locked service images. Every timeline selects one template. Generated histories enforce setup and exclusivity, choose bounded parallel slots, join final timelines, kill live commands for eventual checks, and recover active campaign faults for quiet terminal checks. Template identities, paths, and decisions remain in plans, traces, replay, minimization, and reports. Tutorial 40 is the public runtime path. Production-scale validation remains absent. |
| Ordered execution replay | Implemented, awaiting published runtime evidence | Each vCPU retains a local rolling ledger and every VM retains one bounded machine-wide stream. Fixed runs default to gating the complete stream, including writes, read identities, and supported userspace interrupts, and may select the explicit-host-input contract. Checkpoint-backed campaigns use that host-input contract while retaining intervening exits and interrupt deliveries as evidence; neither host-input path claims identical Linux execution or interrupt timing between those turns. Checkpoint children inherit both ledger forms and pending requests. Read values are checked after device access, not rolled back. Single-service UART events use recorded API input; host-timed pauses are diagnostics, not replayable exits. Tutorial 4 exposes serial replay, Tutorial 41 exposes campaign execution observations, and Tutorial 14 exposes portable container replay. Native PR CI checks the source UART/reset regression; runtime-validation v5 additionally verifies retained standalone run/replay traces and digests. Instruction and Linux scheduling, interrupt servicing, in-kernel timers, and guest input consumption between exits remain uncontrolled. |
...
- Container and Compose tutorials 14–41 expose host image builds, interactive
