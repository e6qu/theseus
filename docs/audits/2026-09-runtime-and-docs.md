...
| Application coverage | Implemented, awaiting native evidence | The packaged GCC frontend emits build-scoped C basic blocks. LLVM frontends add C/C++/Rust edges, independent dynamically loaded module identities, retained symbols, and explicit source lookup. Campaign guidance, timelines, replay, comparison, evaluation, and reports retain the records. Tutorials 31 and 37 are the public runtime paths; automatic report symbolization and Go/Java instrumentation remain absent. |
