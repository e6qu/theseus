...
| Ordered execution evidence | Implemented, awaiting published runtime evidence | Each vCPU retains a local rolling ledger and every VM retains one machine-wide stream. A VM lock serializes handled exits and their emulated device effects. Checkpoint children inherit both forms; replay, comparison, reports, and runtime certification reject divergence. Tutorial 41 is the public ordinary-container path. Host scheduling still selects the next vCPU, and instruction, thread, timer, and interrupt order remain uncontrolled. |
