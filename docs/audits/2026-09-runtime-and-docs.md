...
| Ordered execution replay | Implemented, awaiting published runtime evidence | Each vCPU retains a local rolling ledger and every VM retains one bounded exact machine-wide stream. Checkpoint children inherit both forms and pending level/edge requests. Replay gates each emulated device-effect boundary, explicit UART/control/clock input, and synchronous UART, virtio MMIO, virtio MSI-X, ACPI notification, or keyboard interrupt injection on the recorded actor and payload; comparison, reports, and runtime certification retain the evidence. Tutorial 41 is the public ordinary-container path. Instruction and Linux thread/process order, interrupt servicing, in-kernel timer delivery, and guest-side input consumption between exits remain uncontrolled. |
