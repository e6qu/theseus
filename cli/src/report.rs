...
fn topology(root: &Path) -> Result<ReportModel, ReportError> {
    ...
    for (service, result) in results {
        ...
        if !result.execution_ledgers.is_empty() {
            checks.push(Check {
                name: format!("{service}: ordered KVM execution ledger"),
                ...
                detail: result
                    ...
                    .map(|(vcpu, ledger)| {
                        format!(
                            "vcpu{vcpu}: {} decisions, sha256 {}, last {}",
                            ledger.decisions,
                            ledger.sha256,
                            ledger
                                .tail
...
fn campaign(root: &Path) -> Result<ReportModel, ReportError> {
    ...
    let coverage_signal = match result.coverage.as_str() {
        ...
        "application_edges" => "LLVM-instrumented application edge novelty",
    ...
    Ok(ReportModel {
        ...
        coverage: Some(Coverage {
            ...
            summary: format!(
                ...
                result.execution_decisions,
                result.unique_application_blocks,
                result.unique_application_edges,
...
delta=b=>[['new markers',b.new_markers.join(' ')],['new application coverage',b.new_application_blocks.join(' ')],['scheduling decisions',Object.values(b.new_thread_scheduling_decisions).reduce((n,v)=>n+v.length,0)],['stopped commands',b.terminated_command_services.join(', ')],['changed PCs',b.changed_program_counters.join(', ')],['changed serial',b.changed_serial.join(', ')]].filter(([,value])=>value).map(([label,value])=>label+': '+value).join('; ')||'none',
scheduling=b=>Object.entries(b.new_thread_scheduling_decisions).flatMap(([service,ds])=>ds.map(d=>service+': #'+d.decision+' t'+d.from_thread+' → t'+d.selected_thread+' among '+d.runnable_mask+' @'+d.point_offset)).join(' · ')||'none',
...
fn operation_thread_schedule(operation: &CampaignOperation) -> String {
    ...
    } else if operation.thread_schedule.is_empty() {
        "none".to_owned()
    } else {
        operation
            .thread_schedule
...
fn campaign_timeline_labels(run: &CampaignRun) -> Vec<[String; 20]> {
    run.timeline
        .iter()
        .map(|boundary| {
            ...
            if !boundary.terminated_command_services.is_empty() {
                delta.push(format!(
                    "stopped commands: {}",
                    boundary.terminated_command_services.join(", ")
                ));
            }
            ...
            [
                ...
                campaign_execution_ledger_label(boundary),
...
fn campaign_execution_ledger_label(boundary: &CampaignTimelineBoundary) -> String {
    let mut labels = boundary
        ...
        .flat_map(|(service, ledgers)| {
            ledgers.iter().enumerate().map(move |(vcpu, ledger)| {
                format!(
                    "{service}/vcpu{vcpu}:{} decisions:{}:last {}",
                    ledger.decisions,
                    ledger.sha256,
                    ledger
                        .tail
...
fn campaign_thread_scheduling_label(
    decisions: &BTreeMap<String, Vec<ThreadSchedulingDecision>>,
) -> String {
...
fn render_markdown(model: &ReportModel) -> String {
    ...
    if model
        .campaign_operations
        .iter()
        .any(|operation| operation.shell_phase.is_some() || operation.command.is_some())
    {
        output.push_str(
            "\n## Command lifecycle\n\n| Operation | Template | Test command | Source | Phase | Process | Thread schedule | Structured choices |\n| --- | --- | --- | --- | --- | --- | --- | --- |\n",
        );
        for operation in &model.campaign_operations {
            if operation.shell_phase.is_some() || operation.command.is_some() {
                output.push_str(&format!(
                    "| {} | {} | {} | {} | {} | {} | {} | {} |\n",
                    markdown_cell(&operation.name),
                    markdown_cell(operation.test_template.as_deref().unwrap_or("explicit")),
                    markdown_cell(operation.command.as_deref().unwrap_or("operation")),
                    markdown_cell(operation.test_command_path.as_deref().unwrap_or("explicit")),
                    markdown_cell(operation.shell_phase.as_deref().unwrap_or("input")),
    ...
    if !model.campaign_runs.is_empty() {
        ...
        for run in &model.campaign_runs {
            ...
            output.push_str(&format!(
                "| {} | {} | {} | {} | {}",
                run.index,
                markdown_cell(run.test_template.as_deref().unwrap_or("explicit")),
...
mod tests {
    ...
    fn renders_an_autonomous_compose_campaign() {
        ...
        assert!(markdown.contains("/opt/antithesis/test/v1/main/parallel_driver_write"));
        ...
        assert!(html.contains("parallel_driver"));
        assert!(html.contains("/opt/antithesis/test/v1/main/parallel_driver_write"));
    ...
    #[test]
    fn renders_retained_thread_scheduling_decisions() {
        let directory = tempfile::tempdir().unwrap();
        write_json(
            &directory.path().join("replay-plan.json"),
            r#"{"format":"theseus-compose-plan-v1","campaign":{"operations":[{"name":"deposit","shell_phase":"run","thread_schedule_search":{"threads":[0,1,2],"period":5,"max_switches":3,"generated_schedules":123},"inputs":[{"name":"schedule-0-0-0-1-2","thread_schedule":[0,0,0,1,2]}]}]}}"#,
        );
        ...
        let markdown = report_text(directory.path(), ReportFormat::Markdown).unwrap();
        assert!(markdown.contains(
            "0 structured choices, 1 thread-scheduling decisions, 0 synchronization events, and 0 ordered KVM execution decisions retained"
        ));
        assert!(markdown.contains("Scheduling decisions"));
        assert!(markdown.contains("#4 t1 -> t2 among 0x00000006 @0x42"));
        assert!(markdown.contains("| 0 | explicit | deposit | 0,1,2 |"));
    ...
    fn renders_retained_thread_synchronization_events() {
        ...
        let markdown = report_text(directory.path(), ReportFormat::Markdown).unwrap();
        assert!(markdown.contains(
            "0 structured choices, 0 thread-scheduling decisions, 1 synchronization events, and 0 ordered KVM execution decisions"
        ));
    ...
    #[test]
    fn renders_ordered_kvm_execution_ledgers() {
        let directory = tempfile::tempdir().unwrap();
        write_json(
            &directory.path().join("replay-plan.json"),
            r#"{"format":"theseus-compose-plan-v1","campaign":{"operations":[{"name":"health"}]}}"#,
        );
        let digest = "0123456789abcdef".repeat(4);
        write_json(
            &directory.path().join("campaign-result.json"),
        ...
        let index = report(directory.path(), directory.path().join("report")).unwrap();
        let html = fs::read_to_string(index).unwrap();
        assert!(html.contains("Execution ledger"));
        assert!(html.contains("mmio_read:0x10:1:2a"));
    }
    ...
    #[test]
    fn renders_llvm_application_edges() {
        let directory = tempfile::tempdir().unwrap();
        let digest = "0123456789abcdef".repeat(4);
        write_json(
            &directory.path().join("replay-plan.json"),
            r#"{"format":"theseus-compose-plan-v1","campaign":{"operations":[{"name":"classify"}]}}"#,
        );
        write_json(
            &directory.path().join("campaign-result.json"),
            &format!(
