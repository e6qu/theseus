...
fn campaign_timeline_labels(run: &CampaignRun) -> Vec<[String; 7]> {
    run.timeline
        .iter()
        .map(|boundary| {
...
fn render_markdown(model: &ReportModel) -> String {
    ...
    if !timeline.is_empty() {
        output.push_str("\n## Operation boundaries\n\nEach row is the paused checkpoint after one operation. The delta compares it with the preceding checkpoint. Serial logs and VM snapshots remain in the locked run directory.\n\n| Run | Operation | Delta | Markers | Instruction locations | Applied actions | Serial SHA-256 |\n| --- | --- | --- | --- | --- | --- | --- |\n");
        for [run, operation, delta, markers, locations, actions, serial] in timeline {
            output.push_str(&format!(
                "| {} | {} | {} | {} | {} | {} | {} |\n",
                markdown_cell(&run),
