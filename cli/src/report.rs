...
if(m.campaign_runs.length){{const s=section('Generated timelines');s.append(table(m.campaign_runs.map(r=>[String(r.index),r.operations.join(' → ')||'none',r.fault||'none',r.actions.map(a=>a.kind+' '+a.target).join(' · ')||'none',r.status,r.novelty.join(' ')||'none']),['Run','Operations','Candidate','Applied actions','Status','New markers']));}}
...
mod tests {
    ...
    fn renders_an_autonomous_compose_campaign() {
        ...
        write_json(
            ...
            r#"{"format":"theseus-compose-campaign-result-v1","status":"failed","driver":"api","runs":[{"index":0,"operations":["write","read"],"fault":"backplane:partition@write","actions":[{"kind":"partition","target":"network:backplane"}],"status":"failed","novelty":["42","a1"]}],"properties":[{"name":"consistent_read","kind":"always","status":"failed","detail":"0 of 1 retained timelines contained \"pass\""}]}"#,
        );
        let index = report(directory.path(), directory.path().join("report")).unwrap();
        let html = fs::read_to_string(index).unwrap();
