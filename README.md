...
- Branch-aware per-vCPU ledgers plus one VM-wide ordered stream of handled KVM
  ...
  boundary. Deterministic runs record observed in-kernel timer deliveries
  (the x86 LAPIC timer and aarch64 arch timer) as observation evidence, and an
  opt-in amd64 hold mode injects LAPIC-timer deliveries at recorded vCPU turns
  that replay gates; those deliveries are otherwise uncontrolled.
